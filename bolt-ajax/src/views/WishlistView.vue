<template>
  <div class="container mt-4">
    ...
    <div v-if="botStore.wishlist.length === 0" class="text-center py-5">
    ...
    <div v-else class="row gap-4 justify-content-center">
      <div v-for="p in botStore.wishlist" :key="p._id || p.id" class="card col-12 col-md-4 col-lg-3 p-0 shadow-sm">
        ...
        <div class="card-body">
          ...
          <button @click="botStore.toggleWishlist(p._id || p.id)" class="btn btn-outline-danger w-100">
            Eltávolítás 💔
          </button>
        </div>
      </div>
...
<script setup>
import { useBotStore, BACKEND_URL } from '@/stores/bot.js'
